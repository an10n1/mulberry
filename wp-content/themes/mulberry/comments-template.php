...
function portfolio_comment_template($comment, $args, $depth) {
	...
	<li class="pingback">
		<p>
			...
			<?php edit_comment_link(__('Редактировать', 'portfolio'), '<span class="edit-link">', '</span>'); ?>
		</p>
		<?php break; ?>
	<?php default : ?>
	<?php
		$comment_class = comment_class(empty( $args['has_children'] ) ? '' : 'parent', null, null, false);
		$comment_author = in_array('bypostauthor', explode(' ', $comment_class)) ? ' <strong class="comment-post-author">' . __('Администратор', 'portfolio') . '</strong>' : '';
	?>
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>" <?php $comment_class; ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				...
				<p class="comment-awaiting-moderation"><?php _e('Ваш комментарий проходит модераций.', 'portfolio'); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
